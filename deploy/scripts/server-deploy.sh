...
function detect_compose_file() {
    ...
    local candidate
    for candidate in "${COMPOSE_FILE_CANDIDATES[@]}"; do
        if [ -f "$candidate" ]; then
    ...
    error "未找到可用的 compose 文件！已检查：${COMPOSE_FILE_CANDIDATES[*]}"
    return 1
}

function get_compose_backup_name() {
    basename "$1"
}

function get_compose_restore_target() {
    local file_name="$1"
    if [ -d "deploy/docker" ]; then
        printf 'deploy/docker/%s' "$file_name"
    else
        printf '%s' "$file_name"
    fi
}

function restore_compose_backup_file() {
    local backup_dir="$1"
    local file_name="$2"
    local source_path="$backup_dir/$file_name"

    if [ ! -f "$source_path" ]; then
        warn "备份中未找到 $file_name，跳过"
        return 0
    fi

    local target_path
    target_path="$(get_compose_restore_target "$file_name")"
    mkdir -p "$(dirname "$target_path")"
    cp -f "$source_path" "$target_path"
    info "已恢复 $file_name -> $target_path"
}
...
function backup_current_bundle() {
    ...
    if detect_compose_file; then
        local compose_backup_name
        compose_backup_name=$(get_compose_backup_name "$COMPOSE_FILE")
        cp -f "$COMPOSE_FILE" "$dir/$compose_backup_name"
        info "已备份 $COMPOSE_FILE -> $dir/$compose_backup_name"
    else
...
function rollback_from_backup() {
    ...
    elif [ -f "$dir/$(basename "$IMAGE_TAR")" ]; then
        local tar_backup_name
        tar_backup_name=$(basename "$IMAGE_TAR")
        info "加载备份安装包镜像：$dir/$tar_backup_name"
        docker load -i "$dir/$tar_backup_name"
    else
        error "该备份缺少镜像文件：$dir/image.tar 或 $dir/$(basename "$IMAGE_TAR")"
        return 1
    fi
    ...
    if [[ "$restore_cfg" =~ ^[Yy]$ ]]; then
        ...
        restore_compose_backup_file "$dir" "docker-compose.yml"
        restore_compose_backup_file "$dir" "docker-compose.prod.yml"
    fi
